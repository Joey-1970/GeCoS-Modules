...
class GeCoS_IO extends IPSModule
{
	...
	{ 
		...
		$arrayStatus[] = array("code" => 201, "icon" => "error", "caption" => "Datenverbindung ist gestÃ¶rt");
		...
		$arrayValues[] = array("ServiceTyp" => "IÂ²C", "ServiceStatus" => $ServiceArray["I2C"]["Status"], "rowColor" => $ServiceArray["I2C"]["Color"]);
		...
		If (($this->ConnectionTest()) AND ($this->ReadPropertyBoolean("Open") == true))  {
			// IÂ²C-Devices einlesen und in das Values-Array kopieren
			$DeviceArray = array();
			...
			If (count($DeviceArray , COUNT_RECURSIVE) >= 4) {
				...
				$arrayElements[] = array("type" => "List", "name" => "I2C_Devices", "caption" => "IÂ²C-Devices", "rowCount" => 5, "add" => false, "delete" => false, "sort" => "", "columns" => $arrayColumns, "values" => $arrayValues);
			}
			else {
				$arrayElements[] = array("type" => "Label", "label" => "Es wurden keine IÂ²C-Devices gefunden.");
			}
			$arrayElements[] = array("type" => "Label", "label" => "_____________________________________________________________________________________________________");
			...
			$arrayElements[] = array("type" => "Label", "label" => "_____________________________________________________________________________________________________");
			$arrayElements[] = array("type" => "Label", "label" => "FÃ¼hrt einen Restart des PIGPIO aus:");
		...
		If ($this->ReadPropertyBoolean("Open") == true) {   
			...
			$arrayActions[] = array("type" => "Label", "label" => "Diese Funktionen stehen erst nach Eingabe und Ãœbernahme der erforderlichen Daten zur VerfÃ¼gung!");
		}
	...
	public function ApplyChanges()
	{
		...
		If (IPS_GetKernelRunlevel() == 10103) {				
			...
			If ($ParentID > 0) {
		        ...
		        // Ã„nderung an den untergeordneten Instanzen
		        ...
		        // INSTANCEMESSAGE
		        $this->RegisterMessage($ParentID, 10505); // Status hat sich geÃ¤ndert
			...
			If (($this->ConnectionTest()) AND ($this->ReadPropertyBoolean("Open") == true))  {
				...
				// I2C-Handle zurÃ¼cksetzen
				$this->ResetI2CHandle(0);
				
				// Serial-Handle zurÃ¼cksetzen
				...
				ALT5   = 2
				*/
				// Konfiguration der Interupt-EingÃ¤nge
				$this->CommandClientSocket(pack("L*", 0, 17, 0, 0).pack("L*", 0, 27, 0, 0), 32);
				// Konfiguration der IÂ²C-Pin
				...
				*/
				...
				// Starttrigger fÃ¼r 1-Wire-Instanzen
	...
	public function MessageSink($TimeStamp, $SenderID, $Message, $Data)
		...
		switch ($Message) {
			case 10100:
				If ($Data[0] == 10103) {
					$this->SendDebug("MessageSink", "IPS-Kernel ist bereit und lÃ¤uft", 0);
					$this->ApplyChanges();
				}
				break;
			case 11101:
					...
					if (IPS_SemaphoreEnter("OW", 3000))
					{
						...
						if ($this->OWVerify()) {
							...
							if ($this->OWReset()) { //Reset was successful
								...
								if ($this->OWReset()) { //Reset was successful
									$this->OWSelect();
									$this->OWWriteByte(0x44); //start CÂ° conversion
									IPS_Sleep(10); //Wait for conversion
						...
						If (($Bit17Read == false) AND ($Bitvalue_17 == 0)) {
							...
							If (count($I2CInstanceArray, COUNT_RECURSIVE) >= 5) {
								foreach ($I2CInstanceArray as $Type => $Properties) {
									...
									elseIf (($I2CInstanceArray[$Type]["Notification"] == 2) AND ($I2CInstanceArray[$Type]["DeviceBus"] == 4)) {
										$this->SetMUX(4);
										$Result = $this->CommandClientSocket(pack("L*", 67, $I2CInstanceArray[$Type]["Handle"], 0x0E, 4, 4), 16 + 4);
										$this->SendDataToChildren(json_encode(Array("DataID" => "{573FFA75-2A0C-48AC-BF45-FCB01D6BF910}", "Function"=>"interrupt_with_result_2", "InstanceID" => $I2CInstanceArray[$Type]["InstanceID"], "Value" => $Result)));
									}
								}
							}
	...
	private function CommandClientSocket(String $message, $ResponseLen = 16)
	{
		...
		If (($this->ReadPropertyBoolean("Open") == true) AND ($this->GetParentStatus() == 102)) {
			if (IPS_SemaphoreEnter("ClientSocket", 300))
				...
				// Anfragen mit variabler RÃ¼ckgabelÃ¤nge
	...
	private function ClientResponse(String $Message)
	{
		...
		switch($response[1]) {
			...
			case "40":
		        ...
		        case "54":
		        	If ($response[4] >= 0 ) {
 					//IPS_LogMessage("IPS2GPIO I2C Handle",$response[4]." fÃ¼r Device ".$response[3]);
           			}
           			else {
           				IPS_LogMessage("GeCoS_IO I2C Handle","Fehlermeldung: ".$this->GetErrorText(abs($response[4]))." Handle fÃ¼r Device ".$response[3]." nicht vergeben!");
           			}
			...
			case "57":
		        ...
		        case "59":
					...
					// Keine RÃ¼ckgabe da fÃ¼r Device-Suche!
		            	}
		            	else {
	...
	private function CheckConfig()
	{
		...
		If (($this->ReadPropertyBoolean("Open") == true) AND ($this->GetParentStatus() == 102)) {			
			...
			if (!$sftp->file_exists($PathConfig)) {
				...
				// PrÃ¼fen ob die serielle Schnittstelle aktiviert ist
			...
			if (!$sftp->file_exists($PathCmdline)) {
				...
				// PrÃ¼fen ob die Shell der serielle Schnittstelle aktiviert ist
			...
			$status = @fsockopen($this->ReadPropertyString("IPAddress"), 8888, $errno, $errstr, 10);
				if (!$status) {
					...
					if (!$status) {
						...
						fclose($status);
						//IPS_LogMessage("GeCoS_IO Netzanbindung","Port ist geÃ¶ffnet");
  	...
  	private function SearchI2CDevices()
	{
		...
		for ($j = 4; $j <= 5; $j++) {
			...
			for ($i = 0; $i < count($SearchArray); $i++) {
				// PrÃ¼fen ob diese Device Addresse schon registriert wurde
				$Handle = $this->InstanceArrayHandleSearch($j, $SearchArray[$i]);
				if ($Handle >= 0) {
					// Das GerÃ¤t ist bereits registriert
					// Testweise lesen
					...
					If ($Result >= 0) {
						...
						$DeviceArray[$k][4] = "OK";
						// Farbe grÃ¼n fÃ¼r erreichbare und registrierte Instanzen
						$DeviceArray[$k][5] = "#00FF00";						
					}
					else {
						$DeviceArray[$k][3] = 0;
						$DeviceArray[$k][4] = "Inaktiv";
						// Farbe rot fÃ¼r nicht erreichbare aber registrierte Instanzen
					...
					if ($Handle >= 0) {
						...
						If ($Result >= 0) {
							...
							$DeviceArray[$k][3] = 0;
							$DeviceArray[$k][4] = "OK";
							// Farbe gelb fÃ¼r erreichbare aber nicht registrierte Instanzen
		...
		if ($this->GetBuffer("owLastDevice")) {
			...
			if ($bitNumber == 65) { //if the search was successful then
				...
				If ($OWDeviceArray[$SearchNumber][2] == 0) {
					// Farbe gelb fÃ¼r nicht registrierte Instanzen
