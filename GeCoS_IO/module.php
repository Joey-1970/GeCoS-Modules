...
class GeCoS_IO extends IPSModule
{
			...
			If (($MessageLen == 12) OR (($MessageArray[$i] & 65535) == $this->GetBuffer("NotifyCounter"))) {
				...
				If ($KeepAlive == 1) {
					...
					If ($this->GetBuffer("Serial_Handle") >= 0) {
						...
						$this->SendDebug("Datenanalyse", "Der Serial-Buffer wird nicht geprueft, da kein gueltiger Handle verfügbar ist!", 0);
					}
				}
  	...
  	private function SearchI2CDevices()
	{
		...
		//if (!$fp) {
		//	IPS_LogMessage("SearchI2CDevices", "Fehler beim Verbindungsaufbau ".$errno." ".$errstr);
		//	$this->SendDebug("SearchI2CDevices", "Fehler beim Verbindungsaufbau ".$errno." ".$errstr, 0);
			...
			for ($i = 0; $i < count($SearchArray); $i++) {
				...
				if ($Handle >= 0) {
					...
					*/
					...
					/*
					$Data = pack("L*", 54, 1, $SearchArray[$i], 4, 0);
					stream_socket_sendto($fp, $Data);
					$Buffer = fgets($fp, 16 + 1);
